...
rtk_odom_aekf.py 의 대칭 구현.  RTK 가 lat/lon → ENU 변환을 하는 것과 달리
UWB 는 이미 로컬 프레임 x_m/y_m 을 직접 제공하므로 좌표 변환이 없다.
...
from ..geo_utils import wrap_rad
...
class UwbOdomAEKF:
    ...
    b_θ is not directly observed by UWB, but is estimated indirectly
    ...
    _R_MIN_M2: float = 0.002             # R 하한 (std≈4.5cm) — UWB 실측 정확도 기반
    _R_MAX_M2: float = 25.0              # R 상한 — 이상치 과대적합 방지
    _FORGET_B: float = 0.98              # 망각 인수 b (클수록 과거 가중)
    _WARMUP_STEPS: int = 20              # yaw_calibrated 후 추가 R 고정 스텝 수
    ...
    def __init__(self) -> None:
        ...
        self._prev_uwb: Optional[tuple[float, float]] = None  # 직전 UWB fix
    ...
    def predict(self, odom_x_m: float, odom_y_m: float, odom_yaw_rad: float) -> None:
        ...
        # 홀로노믹 상태 전파: odom 변위를 b_θ 로 UWB 프레임으로 회전
    ...
    def update(self, uwb_x_m: float, uwb_y_m: float) -> bool:
        ...
        if self._prev_uwb is not None and uwb_x_m == self._prev_uwb[0] and uwb_y_m == self._prev_uwb[1]:
            return False
    ...
    def _do_update(self, meas_x_m: float, meas_y_m: float) -> bool:
        ...
        # yaw 가 크게 틀린 상태에서 Sage-Husa 를 켜면 큰 innovation 을 UWB 노이즈로
    ...
    def b_theta_rad(self) -> float:
        """추정된 yaw bias (rad). odom position frame → UWB 프레임 회전 오프셋."""
        return float(self._x[2])

    @property
    def global_yaw_rad(self) -> float:
        """UWB 프레임 기준 절대 heading (rad). odom_yaw + b_θ. 출력용 — propagation 에는 미사용."""
    ...
    def yaw_calibrated(self) -> bool:
        ...
        이동 중 UWB position innovation 이 충분히 누적되면 True 로 전환된다.
